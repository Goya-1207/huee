...
// ── 路径规划 v3（方案 C，无坐标）：最少换乘 → 最少站数 → 尽量晚换乘 ──
// 把字典序代价 (换乘次数, 站数, 早换乘惩罚) 压成一个标量后跑 Dijkstra：
//   · 换乘权重 >> 站数权重 >> 晚换乘微调，三档互不串位；
//   · 「尽量晚换乘」= 坐满当前线再换，能在「换乘数+站数都打平」时
//     避免提前下车去绕远路（正是 锦绣路→龙阳路 兜圈子的成因）。
...
function findRoute(aId, bId) {
  ...
  const W_T = 1e6, W_S = 1e3;                       // 换乘 >> 站数 >> 晚换乘
  const keyOf = (t, s, f) => t * W_T + s * W_S + (300 - f);
  const startLines = linesAtStation(aId);
  if (!startLines.length) return (_routeCache[ck] = null);

  const dist = {}, pq = [];
  const relax = (station, line, t, s, f, prev, via) => {
    const id = station + '|' + line, cost = keyOf(t, s, f);
    if (dist[id] == null || cost < dist[id]) {
      dist[id] = cost;
      pq.push({ station, line, transfers: t, stops: s, firstSeg: f, prev, via, cost });
    }
  };
  for (const L of startLines) relax(aId, L, 0, 0, 0, null, 'start');
...
// 同站多个厕所 → 选最合适的一个
// 优先级：站台 > 付费区站厅 > 站厅(both) > 非付费区站厅 > 站外
function bestToiletAt(st, gender) {
  const ok = st.toilets.filter((t) => meetsNeed(t, gender));
...
function candidateStations(curId, destId) {
  ...
  // 无终点 fallback
  const out = [{ st: byId(curId), travel: 0, isTransfer: false, isCur: true, isDest: false, idx: 0 }];
  const cur = byId(curId);
  if (cur) {
    const adj = adjacency();
    const near = new Set();
    for (const e of adj[curId] || []) near.add(e.to);
    [...near].forEach((sid, k) => {
      const s = byId(sid);
      if (s) out.push({ st: s, travel: PER_HOP, isTransfer: false, isCur: false, isDest: false, idx: 1 + k });
    });
  }
  return out;
}
...
function rankToilets(ctx) {
  ...
  for (const c of cands) {
    const st = c.st;
    const t = bestToiletAt(st, gender);   // ★ 每站只取 1 个厕所 → 去重
    if (!t) continue;
    ...
    // ── ② 位置便携评分（出站代价 + 便利程度，合并计算）──
    // 中途站：站台最优（不出闸不换层）；付费区站厅次之；非付费区/站外需出闸代价极高
    // 终点/当前站：已打算下车，出不出闸无所谓，只考虑步行便利
    const midJourney = !c.isCur && !c.isDest;
    if (t.place === 'platform') {
      score += midJourney ? 28 : 14;
    } else if (t.place === 'concourse' && t.area === '付费区') {
      score += midJourney ? 18 : 9;
    } else if (t.place === 'outside') {
      score += midJourney ? -22 : -2;
    } else {
      // concourse 非付费区 / 站厅(both)——中途使用需出闸
      score += midJourney ? -14 : 3;
    }
